...
from config_from_spreadsheet import SpreadsheetConfigLoader
...
class SEOAnalyzerStreamlit:
    def __init__(self):
        ...
        # スプレッドシートから設定読み込み
        self.config = self.load_config_from_spreadsheet()
        ...
        # Spreadsheet Logger初期化（Drive Loggerの代わり）
        try:
            self.sheet_logger = SpreadsheetLogger(self.config)
            #st.success("✅ スプレッドシート履歴機能を有効化")
        except Exception as e:
    ...
    def save_analysis_result(self, keyword, url, analysis, mode):
        """分析結果を保存（スプレッドシート優先）"""
        # スプレッドシートに保存を試みる
        if hasattr(self, 'sheet_logger') and self.sheet_logger:
            saved_file = self.sheet_logger.save_analysis(keyword, url, analysis, mode)
            if saved_file:
                return f"スプレッドシート保存: {saved_file}"
    ...
    def load_analysis_history(self, site_name=None, limit=20):
        ...
        # スプレッドシートから読み込み
        if hasattr(self, 'sheet_logger') and self.sheet_logger:
            history = self.sheet_logger.load_history(site_name, limit)
            if history:
                return history
...
{competitive_analysis['analysis']}
    ...
    def rewrite_article_with_ai(self, keyword, url, original_content, analysis_text):
        ...
        try:
            ...
            【重要な書式ルール】
            - 「。」で終わる文章ごとに必ず<br>タグを入れて改行すること
            - 「です。」「ます。」「でしょう。」など、句点の後は必ず<br>で改行
            - 箇条書きの場合も各項目の後に<br>を入れる
            - 段落内でも読みやすさのため、2-3文ごとに改行する
            - スマホでの可読性を最優先に考える
            ```
...
def main():
    ...
    with st.sidebar:
        ...
        # サイト選択
        sites = analyzer.config.get('sites', [])
        if not sites:
            st.error("サイトが設定されていません")
            st.stop()
        
        site_names = [site['name'] for site in sites]
        selected_site_name = st.selectbox("分析対象サイト", site_names)
        ...
        with st.expander("詳細設定"):
            change_threshold = st.slider("変化率閾値(%)", 10, 200, 50)
            min_clicks = st.number_input("最小クリック数", 1, 50, 5)
            ...
            if overall_analysis:
                ...
                with col3:
                    ...
                    display_count = st.number_input(
                        "表示件数",
                        min_value=1,
                        max_value=max(1, len(trend_data)),
                        value=min(50, len(trend_data)),
                        step=10,
                        key="trend_display"
                    )
